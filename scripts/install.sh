...
install_ppnode() {
    ...
    if  [[ -z "$version_param" ]] ; then
        last_version=$(curl -Ls "https://api.github.com/repos/Ember-Moth/PPanel-node/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        url="https://github.com/Ember-Moth/PPanel-node/releases/download/${last_version}/ppanel-node-linux-${arch}.zip"
...
EOF
    ...
    curl -o /usr/bin/ppnode -Ls https://raw.githubusercontent.com/Ember-Moth/ppanel-node/master/scripts/ppnode.sh
